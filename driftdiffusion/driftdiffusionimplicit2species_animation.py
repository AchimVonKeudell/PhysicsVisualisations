...
#model = 'ProfileEvolutionGround' # Profileevolution DC
...
if model == 'ProfileEvolutionGround':
    V0 = 0
    dist = 'const' # initial distribution
    dtplot = 1e-10
    simulationtime = 200e-9
    dt=0.5e-10
    simulationfile = 'ProfileGroundAnimation.gif'
...
def voltage(tt):
    ...
    if model == 'ProfileEvolutionGround': # Profileevolution DC
        return 0
...
if dist == 'decay':
  for i in range(1,znb+2):
    ne[i]=np.cos((i-1)/(znb-1)*2*np.pi-np.pi/2)**2*1e14+1e13;
