...
def animate(k):
    ...
    def deriv1(x,t):
        ...
        vd2 = np.array([x[9],x[10],x[11]])
        phit = phi(U,V,t)
        accel = el/mp*Efield(phit,r)
        accel2 = el/mp2*Efield(phit,r2)
        return np.array([vd[0],vd[1],vd[2],accel[0],accel[1],accel[2],vd2[0],vd2[1],vd2[2],accel2[0],accel2[1],accel2[2]])         
    ...
    while time<dtreplot:
        ...
        if timeinteg == 'Euler': # simple euler
          t += dt
          v += el/mp * Efield(phit,x) * dt
          x += dt * v 
          v2 += el/mp2 * Efield(phit,x2) * dt
          x2 += dt * v2
        elif timeinteg == 'rk4':  # 4th order Runge Kutta
          #rk4 step    
          xrk = [x[0],x[1],x[2],v[0],v[1],v[2],x2[0],x2[1],x2[2],v2[0],v2[1],v2[2]]
          #xrk = np.concatenate((x,v,x2,v2))
          #print(xrk)
          half_tau = 0.5*dt
          F1 = deriv1(xrk,t)
          t_half = t + half_tau
          ...
          #x2 = xrkout[6:9] 
          x2 = [xrkout[6],xrkout[7],xrkout[8]]
          #v2 = xrkout[9:12] 
          v2 = [xrkout[9],xrkout[10],xrkout[11]]
          t += dt
